...
function calculateTotal(price: number, tax: number): number {
  ...
  ```
- **Nonlocal Information:** If you must write a comment, then make sure it describes the code it appears near. e.g. rather than pointing to an external source its better to define the content of the source in the comment itself
  ```ts
/**
