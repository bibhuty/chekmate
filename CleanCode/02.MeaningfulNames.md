...
- Pick one word for one abstract concept and stick with it. e.g. `fetch`, `retrieve`, `get` have equivalent meaning and can be used for the same concept but we need to restrain and stick to one of them. Similarly for `controller`, `manager`, `driver` - pick one.
```ts
  // Inconsistent
...
#### Don't pun (avoid using same word for 2 purposes)
- If `add` method is used for the behaviour of concatenating 2 values, when the use case comes to add an existing value into a collection, instead of naming it `add`, we can go for `insert` or `append`.
```ts
  class Calculator {
    add(a: number, b: number): number {
      return a + b;  // 'add' = arithmetic addition
    }
  }
  
  class ShoppingCart {
    // Bad: 'add' is already a pun with arithmetic addition
    add(item: Item) {}
    
    // Good: clearly distinguishes the operation
    append(item: Item) {}
  }
```

#### Use Solution Domain Names
- Use **solution domain names** *(computer science terms — design patterns, algorithms, data structures)* wherever possible. Other programmers recognize them instantly without needing business context.
```ts
  // Design pattern names — instantly recognizable
  class AccountVisitor {}        // Visitor pattern
  class OrderFactory {}          // Factory pattern
  class UserRepository {}        // Repository pattern
  
  // Data structure names imply semantics
  let jobQueue: Task[];          // FIFO implied
  let undoStack: Action[];       // LIFO implied
  let visitedNodes: Set<Node>;   // uniqueness implied
```

#### Use Problem Domain Names (when no solution domain term fits)
- When the code deals with business logic that has no CS equivalent, fallback to **problem domain names** *(domain-specific terms)*. The reader of such code is likely a domain expert.
```ts
  // Insurance domain — no CS equivalents exist
  class PolicyHolder {}
  class ClaimAdjuster {}
  let deductibleAmount: number;
  let coveragePeriod: DateRange;
  
  // Banking domain
  let chargebackWindow: number;  // a banking-specific concept
  let overdraftLimit: number;
```
- The code that has more to do with **problem domain concepts** should have names drawn from problem domains. Infrastructure code leans solution domain; business code leans problem domain - both coexist naturally.
```ts
  class ClaimProcessor {                          // problem domain
    private claimRepository: ClaimRepository;     // solution domain (Repository)
    private notificationQueue: Notification[];    // solution domain (Queue)
    private fraudDetector: FraudDetector;         // problem domain
  }
```